import { IData, IPayload, TCountry } from "@functions/repositories/sqs.repository";
import { SQSService } from "@functions/services/sqs.service";
...
const execute = async (event) => {
  ...
  const queue = getQueueUrl(data);
  const payload: IPayload = getPayload(data);

  try {
    await service.sentMessage(queue, payload);
    return {
      statusCode: 200,
      body: "Message send!",
    };
