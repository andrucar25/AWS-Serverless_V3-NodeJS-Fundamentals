...
export class LambdaService implements LambdaRepository {
  ...
  private getInvokeCommandInput(functionName: string, payload: IPayload, invocationType: InvocationType): InvokeCommandInput{
    return {
  ...
  async invoke(functionName: string, payload: IPayload, invocationType: InvocationType): Promise<void> {
    const args: InvokeCommandInput = this.getInvokeCommandInput(functionName, payload, invocationType);
